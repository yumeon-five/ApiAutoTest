...
ASN_BINSET_QUERY_CASES = get_testcase_yaml('./testcase/asn/asn_binset_query.yaml')


@allure.feature('入库单(ASN)管理')
class TestBinsetQuery:

    @allure.story('查询库位')
    @pytest.mark.parametrize('case_index', range(len(ASN_BINSET_QUERY_CASES)))
    def test_binset_query(self, case_index):
        base_info, testcase = ASN_BINSET_QUERY_CASES[case_index]
