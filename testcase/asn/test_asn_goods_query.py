...
ASN_GOODS_QUERY_CASES = get_testcase_yaml('./testcase/asn/asn_goods_query.yaml')


@allure.feature('入库单(ASN)管理')
class TestGoodsQuery:

    @allure.story('查询可用商品')
    @pytest.mark.parametrize('case_index', range(len(ASN_GOODS_QUERY_CASES)))
    def test_goods_query(self, case_index):
        base_info, testcase = ASN_GOODS_QUERY_CASES[case_index]
