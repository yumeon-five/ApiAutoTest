...
ASN_RETRIEVE_CASES = get_testcase_yaml('./testcase/asn/asn_retrieve.yaml')


@allure.feature('入库单(ASN)管理')
class TestAsnRetrieve:

    @allure.story('按ID查询ASN主单')
    @pytest.mark.parametrize('case_index', range(len(ASN_RETRIEVE_CASES)))
    def test_asn_retrieve(self, case_index):
        base_info, testcase = ASN_RETRIEVE_CASES[case_index]
